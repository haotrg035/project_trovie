...
/***/ "./resources/js/user/admin/articles/index.js":
/*!***************************************************!*\
  !*** ./resources/js/user/admin/articles/index.js ***!
  \***************************************************/
/*! no exports provided */
...
var selectHost = document.querySelector('#select-host');
...
var modalArticle = document.querySelector('#modal-article');
var formArticle = modalArticle.querySelector('form');
var roomInfo = formArticle.querySelector('.room-info');
var roomServiceList = formArticle.querySelector('.room-card__service-list');
var presentRoomCard = document.querySelector('#present-room-card');
var exampleServiceItem = roomServiceList.querySelector('li');
var contentEditor = null;
document.addEventListener('DOMContentLoaded', function () {
  initArticleTable();
  initArticleModal();
  selectFormHandler(selectHost, true); // selectFormHandler(selectRoom);
...
function initArticleTable() {
  ...
  document.querySelector('#btn-create-article').onclick = function () {
  ...
  if (data !== null && data.services.length > 0) {
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        var newServiceItem = exampleServiceItem.cloneNode(true);
...
function initArticleModal() {
  ...
  formArticle.onsubmit = function (e) {
    ...
    axios.post(formArticle.action, data).then(function (response) {
      try {
  ...
  formArticle.querySelector('input[name=title]').oninput = function () {
    var title = formArticle.querySelector('input[name=title]').value.trim();
  ...
  selectRoom.onchange = function () {
    ...
    if (parseInt(selectRoom.value) !== 0) {
      var url = formArticle.querySelector('select[name=room] option[value="' + selectRoom.value + '"]').getAttribute('data-get-room-url');
      axios.get(url, {
        params: {
...
function updateRoomSelectByHostSelect(_hostSelect, _roomSelect) {
  var pickedRoomIds = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
  ...
  try {
    for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
      ...
      if (pickedRoomIds.indexOf(parseInt(_option.value)) < 0) {
...
window.deleteRoomArticle = function (element) {
...
window.refreshRoomArticle = function (element) {
...
window.editRoomArticle = function (element) {
  axios.get(element.getAttribute('data-view-url'), {
    params: {
      api_token: __apiToken
    }
  }).then(function (response) {
    var option = selectRoom.querySelector('option[value="' + response.data.data.room_id + '"]');
...
function renderRowOptionButtons(id, url) {
  return '<button onclick="refreshRoomArticle(this)" title="Làm mới tin" class=" mx-1 btn d-inline-flex btn-sm btn-success rounded" data-id="' + id + '"' + ' data-refresh-url="' + url.refresh + '">' + '<i class="fa fa-refresh" aria-hidden="true"></i>' + '</button>' + '<button onclick="editRoomArticle(this)" class=" mx-1 btn d-inline-flex btn-sm btn-base rounded btn-edit mb-1 mb-lg-0" data-id="' + id + '" ' + ' data-update-url="' + url.update + '" data-view-url="' + url.show + '">' + '<i class="fa fa-pencil-square-o" aria-hidden="true"></i>' + '</button>' + '<button onclick="deleteRoomArticle(this)" class=" mx-1 btn d-inline-flex btn-sm btn-danger rounded btn-delete" data-id="' + id + '"' + ' data-delete-url="' + url["delete"] + '">' + '<i class="fa fa-trash" aria-hidden="true"></i>' + '</button>';
}
...
function renderArticleTableRowRecords(_data, refreshRoomSelect) {
  ...
  var room_ids = _data.map(function (val) {
    return val.room_id;
  });
        ...
        switch (_context.prev = _context.next) {
          case 0:
            ...
            if (select.value === '0') {
              ...
              if (selectHost.value === '0') {
                url = selectHost.dataset.getAllArticlesUrl;
              }
            } else {
...
/***/ 14:
/*!*********************************************************!*\
  !*** multi ./resources/js/user/admin/articles/index.js ***!
  \*********************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/html/project_trovie/resources/js/user/admin/articles/index.js */"./resources/js/user/admin/articles/index.js");
